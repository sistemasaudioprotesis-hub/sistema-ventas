...
export async function GET(request) {
  try {
    ...
    const ventasCompletas = await Promise.all((ventas || []).map(async v => {
      ...
      const { data: cotizData } = await supabase.from('valor_dolar_bna').select('dolar_vendedor').lte('fecha', fechaStr).order('fecha', { ascending: false }).limit(1).maybeSingle()
      ...
      let comisionUSD = 0
      if (derivador) {
        if (derivador.monto_calculado && cotiz) comisionUSD = Number(derivador.monto_calculado) / cotiz
        else if (derivador.tipo_comision === 'monto_fijo' && derivador.valor_comision && cotiz) comisionUSD = Number(derivador.valor_comision) / cotiz
        else if (derivador.tipo_comision === 'porcentaje' && derivador.valor_comision) comisionUSD = precioVentaUSD * Number(derivador.valor_comision) / 100
      }
      ...
      return { ...v, todosEfectivo, factorPago, itemsConSerie, precioVentaUSD, costoUSD, comisionUSD, gananciaBrutaUSD, gananciaNeta, margenPct, tieneAudifonos: itemsConSerie.length > 0 }
    }))
