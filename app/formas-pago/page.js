...
export default function FormasPago() {
  ...
  const [form, setForm] = useState({ forma_pago: '', observaciones: '' })

  useEffect(() => { obtenerFormasPago() }, [])
  ...
  async function guardar() {
    ...
    const body = { forma_pago: nombre, observaciones: form.observaciones || null }
    if (editando) {
      const res = await fetchConToken(`/api/configuracion/formas-pago/${editando.id}`, { method: 'PUT', body: JSON.stringify(body) })
      if (!res.ok) { const d = await res.json(); alert('Error: ' + d.error); return }
      alert('✅ Forma de pago actualizada')
    } else {
      const res = await fetchConToken('/api/configuracion/formas-pago', { method: 'POST', body: JSON.stringify(body) })
      if (!res.ok) { const d = await res.json(); alert('Error: ' + d.error); return }
    ...
    <div style={{ maxWidth: '700px' }}>
      ...
      {mostrarFormulario && (
        <div style={card}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <Field label="Nombre *">
              <input placeholder="Ej: EFECTIVO" value={form.forma_pago} onChange={(e) => setForm({ ...form, forma_pago: e.target.value })} style={{ ...inputStyle, textTransform: 'uppercase' }} />
            </Field>
      ...
      <div style={card}>
        <div style={cardTitle}>
          ...
          <span style={{ marginLeft: '8px', fontSize: '12px', fontWeight: '400', color: '#9ca3af' }}>({formasPago.length} registros)</span>
        </div>
        {formasPago.length === 0 ? (
          <div style={{ color: '#9ca3af', fontSize: '14px', textAlign: 'center', padding: '20px 0' }}>No hay formas de pago cargadas</div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {formasPago.map(f => (
              <div key={f.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', background: '#f9fafb', borderRadius: '8px', border: '1px solid #e5e7eb', flexWrap: 'wrap', gap: '8px' }}>
                <div>
                  <div style={{ fontWeight: '600', fontSize: '15px', color: '#1a1a1a' }}>{f.forma_pago}</div>
                  {f.observaciones && <div style={{ fontSize: '13px', color: '#6b7280', marginTop: '2px' }}>{f.observaciones}</div>}
                </div>
