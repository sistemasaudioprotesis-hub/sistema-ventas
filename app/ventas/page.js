...
export default function Ventas() {
  ...
  async function buscarPacienteAutomatico(dniParam) {
    ...
    if (lista.length === 1) {
      setPaciente(lista[0]); setDni(lista[0].dni)
      setObraSocialId(lista[0].obra_social_id ? String(lista[0].obra_social_id) : '')
      cargarVentasPaciente(lista[0].id)
  ...
  async function guardarAltaRapida() {
    ...
    const res = await fetchConToken('/api/pacientes', {
      method: 'POST',
      body: JSON.stringify({ apellido_paciente: formAltaRapida.apellido, nombres_paciente: formAltaRapida.nombre, dni: formAltaRapida.dni, telefono: formAltaRapida.telefono || null })
    })
    const data = await res.json()
  ...
  async function _agregarItemConfirmado(cantidad) {
    ...
    if (!ventaActualId) {
      const res = await fetchConToken('/api/ventas', {
        method: 'POST',
        body: JSON.stringify({ paciente_id: paciente.id, fecha: new Date().toISOString(), confirmada: false, obra_social_id: obraSocialId ? Number(obraSocialId) : null })
      })
      const data = await res.json()
    ...
    const resDetalle = await fetchConToken(`/api/ventas/${ventaActualId}/detalle`, {
      method: 'POST',
      body: JSON.stringify({
    ...
    if (modoConSerie) {
      await fetchConToken(`/api/stock/series/${form.numero_serie_id}`, {
        method: 'PUT',
        body: JSON.stringify({ en_stock: false, fecha_salida: new Date().toISOString() })
      })
    } else if (controlaStock) {
      await fetchConToken(`/api/stock/productos/${form.producto_id}/movimiento`, {
        method: 'POST',
        body: JSON.stringify({ tipo: 'egreso', cantidad, concepto: `Venta #${ventaActualId}` })
      })
    }

    const prod = productos.find(p => p.id == form.producto_id)
    setItems([...items, {
      id: dataDetalle.detalle.id,
  ...
  async function cargarStockYAgregar() {
    ...
    await fetchConToken(`/api/stock/productos/${modalSinStock.producto_id}/movimiento`, {
      method: 'POST',
      body: JSON.stringify({ tipo: 'ingreso', cantidad: cantidadCargar, concepto: 'Carga manual desde ventas' })
    })
    setStockACargar('')
  ...
  async function eliminarItem(item) {
    ...
    if (item.numero_serie_id) {
      await fetchConToken(`/api/stock/series/${item.numero_serie_id}`, {
        method: 'PUT',
        body: JSON.stringify({ en_stock: true, fecha_salida: null })
      })
    } else if (item.producto_id) {
      const prod = productos.find(p => p.id == item.producto_id)
      if (prod?.controla_stock) {
        await fetchConToken(`/api/stock/productos/${item.producto_id}/movimiento`, {
          method: 'POST',
          body: JSON.stringify({ tipo: 'ingreso', cantidad: item.cantidad || 1, concepto: 'Devolución por eliminación de ítem en venta' })
        })
      }
    }
  ...
  async function guardarDerivador(ventaActualId, totalP, totalU) {
    ...
    await fetchConToken(`/api/ventas/${ventaActualId}/derivador`, {
      method: 'POST',
      body: JSON.stringify({ derivador_id: Number(derivadorId), tipo_comision: tipoComision, valor_comision: Number(valorComision), monto_calculado: montoFinal, pagado: false })
    })
  }
...
async function confirmarYSalir() {
  ...
  async function guardarCambioItem(item, campo, valor) {
    ...
    await fetchConToken(`/api/ventas/${ventaEditando.id}/detalle/${item.id}/historial`, {
      method: 'POST',
      body: JSON.stringify({
    ...
    }).catch(() => {}) // no bloqueamos si la API de historial no existe aún
    ...
    await fetchConToken(`/api/ventas/${ventaEditando.id}/detalle/${item.id}`, {
      method: 'PUT',
  ...
  async function agregarItemEdicion() {
    ...
    const res = await fetchConToken(`/api/ventas/${ventaEditando.id}/detalle`, {
      method: 'POST',
      body: JSON.stringify({
    ...
    if (modoConSerieEdicion) {
      await fetchConToken(`/api/stock/series/${formEdicion.numero_serie_id}`, {
        method: 'PUT',
        body: JSON.stringify({ en_stock: false, fecha_salida: new Date().toISOString() })
      })
    }
  ...
  async function guardarTotalesVenta() {
    ...
    // Guardar historial de la venta antes de modificar
    await fetchConToken(`/api/ventas/${ventaEditando.id}/historial`, {
      method: 'POST',
      body: JSON.stringify({
    ...
    await fetchConToken(`/api/ventas/${ventaEditando.id}`, {
      method: 'PUT',
      body: JSON.stringify({ total_pesos: nuevoTotalPesos, total_dolares: nuevoTotalUSD })
    })

    // Guardar derivador si se eligió uno en edición y no tenía
