...
const btnPrimario = { padding: '10px 20px', background: '#8B1E2D', color: 'white', border: 'none', borderRadius: '8px', fontSize: '15px', fontWeight: '600', cursor: 'pointer', fontFamily: "'Outfit', sans-serif" }    await Promise.all([cargarVentas(), cargarPagos(), cargarCaja()])
    setCargando(false)
  }
  ...
  return (
    <div style={{ maxWidth: '900px' }}>
      ...
      <div style={{ marginBottom: '28px' }}>
        <h1 style={{ fontSize: '26px', fontWeight: '700', color: '#1a1a1a', margin: 0 }}>Reportes</h1>
        <p style={{ color: '#6b7280', marginTop: '4px', fontSize: '14px' }}>Ventas, pagos y caja por período</p>
      </div>
  ...
  <select value={obraSocialId} onChange={(e) => setObraSocialId(e.target.value)} style={inputStyle}>
    ...
    {obrasSociales.map(o => <option key={o.id} value={o.id}>{o.obra_social}</option>)}
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
        {[['ventas', '📊 Ventas'], ['pagos', '💳 Pagos'], ['caja', '💰 Caja']].map(([val, label]) => (
          <button key={val} onClick={() => setTab(val)} style={{
            padding: '9px 20px',
            borderRadius: '8px',
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px' }}>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        ...
                        <span style={{
                          ...
                          background: v.confirmada ? '#dcfce7' : '#fef9c3',
                          color: v.confirmada ? '#16a34a' : '#ca8a04',
                        }}>
                          {v.confirmada ? 'Confirmada' : 'Pendiente'}
                        </span>
                      </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {movimientosCaja.map(m => (
                  <div key={m.id} style={{
                    ...
                    <div style={{ fontWeight: '700', fontSize: '14px', color: m.tipo === 'ingreso' ? '#16a34a' : '#dc2626' }}>
                      {m.tipo === 'egreso' ? '-' : ''}
                      {m.monto_pesos ? fmt(m.monto_pesos) : fmtUSD(m.monto_usd)}
                    </div>
                  </div>
                ))}
              </div>
