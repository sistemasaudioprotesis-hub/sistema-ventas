...
export default function ConfiguracionProductos() {
  ...
  const [formProducto, setFormProducto] = useState({ producto: '', tipo_id: '', controla_stock: false, activo: true })
  ...
  async function obtenerDatos() {
    const [resTipos, resProductos] = await Promise.all([
      fetchConToken('/api/configuracion/tipos-producto'),
      fetchConToken('/api/configuracion/productos'),
    ])
    const [dTipos, dProductos] = await Promise.all([resTipos.json(), resProductos.json()])
  ...
  function abrirEditarTipo(t) {
    setEditandoTipo(t)
    setFormTipo({ tipo: t.tipo || '', requiere_serie: t.requiere_serie ?? true })
    setMostrarFormTipo(true)
  }

  function cerrarFormTipo() {
  ...
  async function guardarTipo() {
    ...
    if (editandoTipo) {
      await fetchConToken(`/api/configuracion/tipos-producto/${editandoTipo.id}`, { method: 'PUT', body: JSON.stringify(body) })
    } else {
      const res = await fetchConToken('/api/configuracion/tipos-producto', { method: 'POST', body: JSON.stringify(body) })
  ...
  function abrirNuevoProducto() {
    ...
    setFormProducto({ producto: '', tipo_id: '', controla_stock: false, activo: true })
    setMostrarFormProducto(true)
  }

  function abrirEditarProducto(p) {
    setEditandoProducto(p)
    setFormProducto({ producto: p.producto || '', tipo_id: String(p.tipo_id || ''), controla_stock: p.controla_stock || false, activo: p.activo ?? true })
    setMostrarFormProducto(true)
  }
  ...
  async function guardarProducto() {
    ...
    const body = { producto: normalizarTexto(formProducto.producto), tipo_id: Number(formProducto.tipo_id), controla_stock: formProducto.controla_stock, activo: formProducto.activo }
    if (editandoProducto) {
      await fetchConToken(`/api/configuracion/productos/${editandoProducto.id}`, { method: 'PUT', body: JSON.stringify(body) })
    } else {
      const res = await fetchConToken('/api/configuracion/productos', { method: 'POST', body: JSON.stringify(body) })
      if (!res.ok) { const d = await res.json(); alert('Error: ' + d.error); return }
    }
    cerrarFormProducto()
    obtenerDatos()
  ...
  return (
    <div style={{ maxWidth: '720px' }}>
      ...
      <div style={{ marginBottom: '28px' }}>
        ...
        <p style={{ color: '#6b7280', marginTop: '4px', fontSize: '14px' }}>ABM de tipos de producto y productos</p>
      </div>

      {/* Tabs */}
      <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
        {[['tipos', '🏷️ Tipos de producto'], ['productos', '📦 Productos']].map(([val, label]) => (
      ...
      {tab === 'tipos' && (
        <>
          ...
          <div style={card}>
            ...
            {tipos.length === 0 ? (
              <div style={{ color: '#9ca3af', fontSize: '14px', textAlign: 'center', padding: '20px 0' }}>No hay tipos registrados</div>
            ) : (
                  ...
                  <div key={t.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', background: '#f9fafb', borderRadius: '8px', border: '1px solid #e5e7eb' }}>
                    <div>
                      ...
                      <div style={{ fontSize: '12px', color: '#6b7280', marginTop: '2px' }}>
                        {t.requiere_serie ? '🔢 Requiere número de serie' : '📦 Sin número de serie'}
                      </div>
                    </div>
      ...
      {tab === 'productos' && (
        <>
          ...
          <div style={card}>
            ...
            {productos.length === 0 ? (
              <div style={{ color: '#9ca3af', fontSize: '14px', textAlign: 'center', padding: '20px 0' }}>No hay productos registrados</div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {productos.map(p => (
                  <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', background: p.activo ? '#f9fafb' : '#fafafa', borderRadius: '8px', border: '1px solid #e5e7eb', opacity: p.activo ? 1 : 0.6 }}>
                    <div>
                      ...
                      <div style={{ fontSize: '12px', color: '#6b7280', marginTop: '2px' }}>
                        ...
                        {p.controla_stock ? ' · 📊 Controla stock' : ''}
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: '8px' }}>
                      ...
                      <button onClick={() => toggleActivoProducto(p)} style={{ ...btnSecundario, fontSize: '12px', padding: '6px 12px', color: p.activo ? '#dc2626' : '#16a34a', borderColor: p.activo ? '#fecaca' : '#bbf7d0' }}>
